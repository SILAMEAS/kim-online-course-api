...
public class UserService extends AbstractCrudCommon<User, Long, UserRepository> {
  ...
  public EntityResponseHandler<UserResponse.UserResponseCustom> getUsersWhoOrderedFromRestaurant(
      Long restaurantId, Pageable pageable) {
    Specification<User> spec = UserSpec.hasOrderedFromRestaurant(restaurantId);
    return new EntityResponseHandler<>(
        super.findAll(spec, pageable).map(UserResponse::toUserResponseCustom));
  }

  public Long countById(Long restaurantId) {
    return this.baseRepository.count(UserSpec.hasOrderedFromRestaurant(restaurantId));
  }
