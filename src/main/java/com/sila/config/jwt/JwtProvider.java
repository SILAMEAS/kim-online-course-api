...
public class JwtProvider {
  ...
  public String generateToken(Authentication auth) {
    ...
    return Jwts.builder()
        ...
        .setExpiration(
            new Date(new Date().getTime() + JwtConstant.ACCESS_TOKEN_EXPIRATION)) // 1 day
  ...
  public String generateRefreshToken(Authentication auth) {
    return Jwts.builder()
        .setIssuer(String.valueOf(new Date()))
        .setExpiration(
            new Date(new Date().getTime() + JwtConstant.REFRESH_TOKEN_EXPIRATION)) // 7 days
        .claim("email", auth.getName())
        .signWith(key)
        .compact();
  }
  ...
  public long getExpirationIn(Long milliseconds) {
